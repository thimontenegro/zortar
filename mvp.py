        ...
        if df_concorrente.shape[0] != 0:
            ...
            for app in aplicativos_concorrentes:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    y = app_atual[0:train_size]['position'], mode = 'lines', name = f'Observados do {app}', opacity = 0.45, line = dict(color=colors[i])
                ))
                fig.add_trace(go.Scatter(
                    x = app_atual['ds'][-10:],
                    y = app_atual['yhat'][-10:].values, mode = 'lines', name = f"Previsão {app}", connectgaps=True, opacity = 0.45, line = dict(color = colors_prediction[i], dash = 'dot')
                ))
                i += 1
